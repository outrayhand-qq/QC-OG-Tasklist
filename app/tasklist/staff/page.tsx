...
const STAFF_PICS = ['Staff QC', 'Staff Klaim', 'Staff CSO', 'Staff CC']
...
export default function StaffConsole() {
  ...
  const staffScopedTasks = useMemo(() => {
    return tasks.filter((t) => {
      const isStaffTask = STAFF_PICS.some((sp) => t.pic_assignment?.includes(sp))
      const matchesPic = selectedPicFilter === 'All' || t.pic_assignment?.includes(selectedPicFilter)
      return isStaffTask && matchesPic
    })
  }, [tasks, selectedPicFilter])

  const stats = useMemo(() => {
    const now = new Date()
    const total = staffScopedTasks.length
    const open = staffScopedTasks.filter((t) => t.status === 'Open').length
    const progress = staffScopedTasks.filter((t) => t.status === 'On Progress').length
    const closed = staffScopedTasks.filter((t) => t.status === 'Closed').length
    const urgent = staffScopedTasks.filter((t) => t.priority === 'Urgent' && t.status !== 'Closed').length
    const overdue = staffScopedTasks.filter((t) => {
  ...
  const filteredTasks = useMemo(() => {
    return staffScopedTasks.filter((t) => {
  ...
  }, [staffScopedTasks, selectedKategori, selectedStatus, selectedPriority, searchQuery])
    ...
    <main className="p-8 max-w-[1600px] w-full mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 pb-2">
        <div>
          <h1 className="text-xl font-extrabold tracking-tight text-zinc-900">
            Staff Task Management Console
          </h1>
          <p className="text-xs text-zinc-500 mt-1">
            Konsol operasional harian untuk staff QC, Klaim, CSO, dan CC.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => exportTasksToCSV(filteredTasks, 'Tasklist_Staff')}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-2.5 rounded-lg border border-zinc-200/80 shadow-2xs">
        <div className="flex items-center gap-1 overflow-x-auto p-0.5 text-xs font-medium scrollbar-none">
          <button
            ...
            className={`px-3 py-1.5 rounded transition whitespace-nowrap cursor-pointer ${selectedKategori === 'All' ? 'bg-zinc-100 text-zinc-900 font-bold border border-zinc-200/80' : 'text-zinc-600 hover:text-zinc-900'}`}
          >
            Semua Kategori
          </button>
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded transition whitespace-nowrap cursor-pointer ${selectedKategori === cat ? 'bg-zinc-100 text-zinc-900 font-bold border border-zinc-200/80' : 'text-zinc-600 hover:text-zinc-900'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="bg-white border border-zinc-200/90 rounded-lg shadow-2xs overflow-hidden">
        {loading ? (
          <div className="py-20 text-center text-xs text-zinc-400 font-bold uppercase tracking-widest animate-pulse">
            Memuat Data Staff Tasklist...
          </div>
        ) : filteredTasks.length === 0 ? (
          <div className="py-20 text-center text-xs text-zinc-500 font-medium space-y-1">
            <p className="font-bold text-zinc-700">Tidak ada task staff yang ditemukan</p>
          </div>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-zinc-200 w-full max-w-lg rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-zinc-200 flex items-center justify-between bg-white">
              <h3 className="font-extrabold text-sm text-zinc-900">Tambah Task Staff Baru</h3>
            ...
            <form onSubmit={handleAddTask} className="p-5 space-y-4 overflow-y-auto text-xs">
              <div className="space-y-1.5">
                <label className="block font-bold text-zinc-900">Assign Staff PIC</label>
      ...
      {editingTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-zinc-200 w-full max-w-lg rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-zinc-200 flex items-center justify-between bg-white">
              <h3 className="font-extrabold text-sm text-zinc-900">Edit Staff Task</h3>
            ...
            <form onSubmit={handleUpdateTask} className="p-5 space-y-4 overflow-y-auto text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  <label className="block font-bold text-zinc-500 flex items-center gap-1"><span>🔒</span> Deadline (Terkunci)</label>
